...
sealed class TokenAlgorithm(internal val internal: String) {
    abstract suspend fun sign(key: String, payload: ByteArray): ByteArray?
    abstract suspend fun verify(key: String, payload: ByteArray, signature: ByteArray?): Boolean

    object RS256 : TokenAlgorithm("RS256") {
        override suspend fun sign(key: String, payload: ByteArray): ByteArray = signSha256WithRSA(key, payload)
        override suspend fun verify(key: String, payload: ByteArray, signature: ByteArray?): Boolean =
    ...
    object None : TokenAlgorithm("None") {
        override suspend fun sign(key: String, payload: ByteArray): ByteArray? = null
        override suspend fun verify(key: String, payload: ByteArray, signature: ByteArray?): Boolean = true
    }

    class Other(internal: String) : TokenAlgorithm(internal) {
        override suspend fun sign(key: String, payload: ByteArray): ByteArray? = null
        override suspend fun verify(key: String, payload: ByteArray, signature: ByteArray?): Boolean = false
    }
}
