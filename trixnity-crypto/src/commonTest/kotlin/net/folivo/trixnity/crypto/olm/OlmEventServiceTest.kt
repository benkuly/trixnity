...
private val body: ShouldSpec.() -> Unit = {
    ...
    should("encrypt without stored olm encrypt session") {
        ...
        mockStore.olmSessions[bobCurveKey] shouldNotBe null
    }
    should("throw exception when one time key is invalid without stored olm encrypt session") {
        ...
        mockStore.olmSessions.shouldBeEmpty()
    }
    should("encrypt event with stored session") {
        freeAfter(
            ...
            mockStore.olmSessions[bobCurveKey] = setOf(storedOlmSession)
            ...
            mockStore.olmSessions[bobCurveKey]?.firstOrNull().shouldNotBeNull() shouldNotBe storedOlmSession
        }
    }
    // #######################
    // decryptOlm
    ...
    should("decrypt pre key message from new session") {
        ...
        mockStore.olmSessions[bobCurveKey].shouldNotBeNull() shouldHaveSize 1
    ...
    should("not decrypt pre key message, when the 5 last created sessions are not older then 1 hour") {
        ...
        mockStore.olmSessions[bobCurveKey] = existingSessions
    ...
    should("try multiple sessions descended by last used") {
        freeAfter(
            ...
            mockStore.olmSessions[bobCurveKey] = setOf(
                storedOlmSession2,
                storedOlmSession1,
                storedOlmSession3
            )
            ...
            mockStore.olmSessions[bobCurveKey].shouldNotBeNull() shouldNotContain storedOlmSession1
        }
    }
    ...
    suspend fun shouldEncryptMessage(
        ...
        val storedOutboundSession = mockStore.outboundMegolmSession[room]
        ...
        val storedInboundSession = mockStore.inboundMegolmSession[sessionId to room]
    ...
    suspend fun createExistingOutboundSession() {
        freeAfter(OlmOutboundGroupSession.create()) { outboundSession ->
            mockStore.outboundMegolmSession[room] =
            ...
            freeAfter(OlmInboundGroupSession.create(outboundSession.sessionKey)) { inboundSession ->
                mockStore.inboundMegolmSession[outboundSession.sessionId to room] = StoredInboundMegolmSession(
    ...
    should("decrypt megolm event") {
        freeAfter(OlmOutboundGroupSession.create()) { outboundSession ->
            ...
            mockStore.inboundMegolmSessionIndex[Triple(outboundSession.sessionId, room, 0)] shouldBe
    ...
    should("handle manipulated message index") {
        freeAfter(OlmOutboundGroupSession.create()) { outboundSession ->
            ...
            val ciphertext =
                outboundSession.encrypt(json.encodeToString(decryptedMegolmEventSerializer, decryptedMegolmEvent))
            mockStore.inboundMegolmSessionIndex[Triple(outboundSession.sessionId, room, 0)] =
                StoredInboundMegolmMessageIndex(
