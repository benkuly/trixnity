...
import net.folivo.trixnity.appservice.rest.api.MatrixServerException
...
fun Route.v1(appserviceHandler: AppserviceHandler) {
    ...
    put("/transactions/{tnxId}") {
        val tnxId = call.parameters["tnxId"]
    ...
    get("/users/{userId}") {
        ...
        try {
            ...
            if (hasUser) call.respond(HttpStatusCode.OK) else throw MatrixNotFoundException("user $userId not found")
    ...
    get("/rooms/{roomAlias}") {
        ...
        try {
            ...
            return if (hasRoomAlias) call.respond(HttpStatusCode.OK) else throw MatrixNotFoundException("no room alias $roomAlias found")
        } catch (error: Throwable) {
            if (error !is MatrixServerException) {
                throw MatrixNotFoundException(error.message ?: "unknown")
            } else {
