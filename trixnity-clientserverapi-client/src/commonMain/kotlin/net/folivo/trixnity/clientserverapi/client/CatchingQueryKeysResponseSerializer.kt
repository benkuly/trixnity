                    ...
                    .mapValues {
                        it.value.mapNotNull { (device, deviceKeyJson) ->
                            try {
                                ...
                                log.warn(e) { "Could not deserialize device keys. It will be ignored." }
                                null
                            }
                        }.toMap()
                    }
        ...
        decoder.json.decodeFromJsonElement<Map<UserId, JsonObject>>(crossSigningKeysJson)
            .mapNotNull { (userId, crossSigningKeyJson) ->
                try {
                    ...
                    log.warn(e) { "Could not deserialize cross singing key. It will be ignored." }
                    null
                }
            }.toMap()
    }

    override fun serialize(encoder: Encoder, value: QueryKeysResponse) {
